...
import { HandlerFn, VerifyPayloadType, WebhookPayload, WebhookRegistrationConfig, WebhooksHandler, WebhooksHandlerMap } from "./types"
...
export const verifyPayload = ({ payload, signature }: VerifyPayloadType) => {
    const publicKey = Buffer.from(constants.WH_PUBLIC_KEY, 'base64').toString('ascii');
    ...
    const isValid = verify.verify(publicKey, signature, constants.WH_SIGNATURE_FORMAT);
...
export async function handleWebhooks(config: WebhookRegistrationConfig, req: Request): Promise<Response> {
  ...
  if(!verifyPayload({ payload: JSON.stringify(payload), signature: webhook_signature})) return new Response('Error occured', {
    status: 400
  })
  ...
  if(handlerMap[payload.event]) {
    return await _handler(payload, handlerMap[payload.event] as HandlerFn<WebhookPayload>)
  }

  return new Response('', { status: 404 })
}

async function _handler(event: WebhookPayload, callback: Function): Promise<Response> {
